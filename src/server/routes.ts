...
export const routes = async function (app: express.Express) {
    ...
    // custom routes
    app.get ('/api/corsproxy', await loadModule('./routes/corsproxy', 'corsProxyGet'));
    app.post('/api/corsproxy', await loadModule('./routes/corsproxy', 'corsProxyPost'));
    app.use('/mining', await loadModule('./routes/freemining'));
